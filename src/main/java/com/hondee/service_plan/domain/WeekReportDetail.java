...
@Table(name = "plan_weekreportdetail")
public class WeekReportDetail extends BaseEntity {
    @ApiModelProperty("周汇报清单项编号")
    @Id
    String weekReportDetailCode;

    @ApiModelProperty("周汇报")
    @JoinColumn(name = "weekReportCode")
    @ManyToOne
    ...
    @ManyToOne
    WeekReportDetail parent;

    @ApiModelProperty("子项")
    @OneToMany(mappedBy = "parent", cascade = CascadeType.ALL, fetch = FetchType.LAZY, orphanRemoval = true)
    @OrderBy("sequence asc")
    List<WeekReportDetail> children;
    ...
    @ApiModelProperty("明细")
    @OneToMany(mappedBy = "weekReportDetail", cascade = CascadeType.ALL, fetch = FetchType.LAZY, orphanRemoval = true)
    @OrderBy("periodDay asc")
    List<WeekReportTracking> trackings;
}
