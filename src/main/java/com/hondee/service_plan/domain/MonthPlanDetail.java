...
@Table(name = "plan_monthplandetail")
public class MonthPlanDetail extends BaseEntity {
    @ApiModelProperty("月度计划清单项编号")
    @Id
    String monthPlanDetailCode;

    @ApiModelProperty("月度计划")
    @JoinColumn(name = "monthPlanCode")
    @ManyToOne
    MonthPlan monthPlan;
    ...
    @ManyToOne
    MonthPlanDetail parent;

    @ApiModelProperty("子项")
    @OneToMany(mappedBy = "parent", cascade = CascadeType.ALL, fetch = FetchType.LAZY, orphanRemoval = true)
    @OrderBy("sequence asc")
    List<MonthPlanDetail> children;
