...
public class ExcelUtil {
    ...
    private List<Object> handlerExcel2Objs(Workbook wb, Class clz, int readLine, int tailLine) {
        ...
        try {
            ...
            if (maps.size() <= 0) throw new RuntimeException("要读取的Excel的格式不正确，检查是否设定了合适的行");
            for (int i = readLine + 1; i <= sheet.getLastRowNum() - tailLine; i++) {
                ...
                for (Cell c : row) {
                    ...
                    mn = mn.substring(0, 1).toLowerCase() + mn.substring(1);
                    try {
                        if (clz.getDeclaredField(mn).getType().getSimpleName().equals("Date")) {
                            if (this.getCellValue(c).length() > 10) {
                                BeanUtils.copyProperty(obj, mn, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(this.getCellValue(c)));
                            } else {
                                BeanUtils.copyProperty(obj, mn, new SimpleDateFormat("yyyy-MM-dd").parse(this.getCellValue(c)));
                            }
                        } else {
